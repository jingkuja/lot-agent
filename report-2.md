# Lot Agent 全面 Review 报告（修订版）

> Review 范围：`packages/core`（Agent 引擎）、`packages/server`（HTTP / DB / Worker）、`packages/web`（Workspace UI），约 2.7 万行 TypeScript / TSX。
>
> 说明：仓库包含 93 个测试文件；原报告记录为 610 个用例全部通过。本次修订基于静态代码审阅，没有重新执行测试，因此不独立确认该测试结果。建议在报告中同时记录测试命令、commit 和执行时间。
>
...
`packages/server/src/services/agent-service.ts:727-733` 的用量上报使用 `def.defaultModelId`，但本回合真正使用的模型来自显式选择、会话存储或 Agent 默认值的解析结果。
...
- 高价模型可能按低价默认模型结算，造成资损；
- `usage_logs.model_id` 本身错误，按模型统计的报表失真；
- trace 中记录的 model/provider 也可能沿用默认模型，而非本回合实际模型。

仓库中的精确 Claude 模型 ID 是 `claude-sonnet-4-20250514`。价格差异应以当前配置或动态模型目录为依据，不应使用未被仓库配置佐证的外部价格断言。
...
- `GET /api/traces` 不传 `conversationId` 时会返回全站最近 trace；
- `GET /api/traces/:id` 可读取任意 trace 及 spans；
- 没有执行 conversation → user 的属主校验。

泄露内容包括会话关联、模型字段、token、延迟、错误信息及工具名称/顺序。需要注意：当前持久化 trace 并不包含 `totalCost`；`packages/server/src/services/trace-recorder.ts:73-75` 明确说明该字段未被持久化，而成本是在 `finish()` 之后写入内存对象。因此不能把 `totalCost` 列为当前接口已经泄露的字段。
...
重新加载历史时，`packages/server/src/services/message-repository.ts:59-64` 又会过滤缺少结果的 tool call，因此第 2..N 个工具调用及结果从历史中消失。

根因是 `AgentEvent` 的 `tool_result` 只有工具名称，没有 `toolCallId`（`packages/core/src/agent.ts:30`），服务端只能按名称猜测配对；同名并行调用必然存在歧义。

建议让 `tool_result` 强制携带 call ID，服务端按 ID 逐项持久化，并补充“多个不同名工具”和“多个同名工具”两类并行回归测试。

#### 7. 图片/视频的预检、执行与结算没有使用同一实际模型

- `packages/server/src/routes/conversations.ts:271-280` 和 `packages/server/src/routes/tasks.ts:30-39` 使用固定模型估价，用户实际提交的模型只被透传给 worker。
- `packages/server/src/workers/index.ts:123` 与 `packages/server/src/generation/run-job.ts:120-125` 又按配置默认模型结算。
...
- `packages/server/src/generation/run-job.ts:54` 没有接收 signal；
...
- `packages/server/src/db/database.ts:1408-1421` 的成功/失败更新可覆盖 `cancelled`；
...
缓存命中时，worker 可能先写入 `{status: "completed", assets}`，随后 route 再把消息覆盖回 `generating` 并丢掉 assets。刷新后消息可能长期停留在生成中。

应将 task 行作为单一事实来源；如果保留 metadata 冗余，则使用 JSONB merge、版本/CAS 和单向状态迁移守卫。

#### 10. 配额检查覆盖不全，而且“先查后调用”可并发超卖

`checkQuota` 目前主要用于 image/video，聊天、标题、上下文压缩等昂贵调用没有统一配额入口。即使把所有调用包进简单的 `withQuota()`，多个并发请求仍可能同时读取旧消费值并全部通过。
...
`packages/server/src/generation/run-job.ts:76-85` 的缓存 key 不包含 user ID。用户 B 使用相同输入命中用户 A 的缓存时：
...
`documents` 路由会以内联 `text/html` 返回 HTML。`nosniff` 不能阻止明确声明为 `text/html` 的内容执行，因此仅补该响应头不够。建议选择以下方案之一：
...
当前 session token 存在 localStorage，一旦同源脚本执行，token 可能被读取，因此文件源隔离尤为重要。

#### 14. 静态文件全量读入内存且不支持 Range

`packages/server/src/index.ts:184,197,210` 都使用 `readFile` 返回完整 Buffer。大文件并发下载会增加内存压力，视频也无法可靠拖动进度。

建议改为流式响应，支持 `Range`、`Accept-Ranges`、`Content-Range`、合理缓存头，并对单次与并发带宽做限制。

#### 15. API key、平台 access token 和 session token 明文存储

- `packages/server/src/db/database.ts:1026-1031` 涉及用户 tokenhub API key；
- `sessions.token` 保存原始 session token；
...
`packages/server/src/auth/session-store.ts:4,16-23` 采用 7 天硬过期并更新 `last_seen_at`，但过期行不会自动清理。
...
`packages/server/src/db/database.ts:786-794` 使用传入 message ID 的 `created_at` 作为删除边界，但没有验证该消息属于目标会话。路由虽然校验了会话属主，却未验证消息归属。

传入其他会话的消息 ID 时，可能按错误时间点删除当前用户自己的消息。建议 SQL 同时约束 `message.id` 与 `conversation_id`，找不到时返回 404。

#### 18. worker 与 server 的模型价格目录不一致

worker 的 `UsageMeter` 只查询静态配置（`packages/server/src/workers/index.ts:65`），server 侧则支持动态 catalog 兜底。memory extraction 使用动态模型 ID 时，worker 可能把它视为 unknown model 并跳过计费。

server 和 worker 应共享同一个 ModelCatalog / PricingResolver，并明确动态目录缓存的一致性和失效机制。

#### 19. 工具执行的取消、超时和路径边界不统一

- `web_fetch` 忽略 `context.signal`，使用私有 timeout controller，run 取消后仍可能继续请求。
- `search_files` 的 grep 参数缺少 `--` 分隔，`-` 开头的 pattern 可能被解释为选项。
- `withLLMRetry` 的退避 sleep 不可中断。
- `ToolRegistry.timeout()` 的 timer 没有及时清理。
- 文件工具使用 `resolve` 做路径 containment，但不解析符号链接；工作目录内 symlink 可能指向目录外。

建议统一使用可组合、可清理的 signal/timeout helper；文件访问在最终打开目标前验证 realpath，并根据部署模式决定是否彻底禁用符号链接。

#### 20. 同一会话的服务端并发无保护

web 端只在单个页面实例中限制同会话单流，但两个标签页或设备仍可同时发送。服务端没有会话级并发控制，消息可能交错写入；当前按 `created_at` 排序也无法保证同时间戳记录的稳定顺序。
...
login、uploads、消息发送、生成任务等路径没有完整 rate limit。上传还可能把 20MB body 整体读入内存。

建议按 IP、用户和接口成本分层限流；上传使用流式解析并限制并发、文件数、总字节与 MIME/magic bytes；昂贵调用同时受频率限制和额度 reservation 约束。

#### 22. Redis 任务进度发布没有消费者

`packages/server/src/queue/bullmq-queue.ts:172-178` 发布 `task:{id}:progress`，但当前没有订阅端；web 每秒轮询 task API。
...
| # | 问题 | 位置 / 建议 |
|---|---|---|
| 23 | `maxRunTimeMs: 600_000` 与 “5 min” 注释、默认值文档不一致 | `packages/core/src/agent.ts:44,102`，统一常量和注释 |
...
| 26 | malformed-recovery 合成消息只存在 working history | `packages/core/src/agent.ts:423`；主要影响 trace/replay 可复现性，若保存应作为内部事件或 metadata，而非伪装成普通 user 消息 |
| 27 | 标题 prompt 规定 30 chars，代码截断 50 | `packages/server/src/services/agent-service.ts:467,476` |
| 28 | 前端消息 ID 基于 `Date.now()`，同毫秒事件可能撞 key | `packages/web/src/hooks/useChat.ts:227,306`，使用事件 ID/toolCallId/UUID |
| 29 | regenerate 先删除 DB 历史再重发，失败时不可恢复 | `packages/web/src/hooks/useChat.ts:399-408`，改为分支或事务化状态 |
| 30 | 上传 MIME 完全信任客户端 `File.type` | `packages/server/src/routes/uploads.ts:31`，增加 magic-bytes 检测 |
| 31 | 多张业务表的 `user_id` 使用 VARCHAR，缺乏系统性外键约束 | 不只 `usage_logs`、`conversations`，还应检查 tasks、assets、review_logs、platform_accounts、publish_records、user_agents 等 |
| 32 | `anySignal` 给外部 signal 增加的监听器未移除 | `packages/core/src/agent.ts:87-98` |
| 33 | `web_search` 使用平台 `BIGMODEL_API_KEY`，成本未计量或限流 | `packages/core/src/tools/builtins.ts:479`；它是外部搜索成本，不宜直接描述为违反“所有模型调用使用用户 key” |
...
`tool_result` 必须携带 `toolCallId`，服务端按 ID 配对持久化。所有流式事件可进一步携带：
...
增加 `seq` 只是断线续传的前提，不等于已经支持 replay。

### 3. 计费收敛为统一的 metered execution 管道

建议用一个统一执行器覆盖 chat、title、compression、memory、image 和 video：
...
`modelId` 应来自 provider/run context，不允许每个调用点手填。server 与 worker 必须共享同一个 PricingResolver。

### 4. 任务状态使用单一事实来源和显式状态机

建议 task 表作为权威状态源，message metadata 只保存 `taskId`。状态迁移至少应定义：
...
内部统一采用 404 隐藏资源存在性。为 conversations、messages、ratings、traces、tasks、assets 和 downloads 建立跨用户访问测试矩阵。

### 6. 拆分 `AgentService`

`AgentService` 同时承担 DB、Redis、BullMQ、SkillLoader、MCP、catalog、provider、usage、上传等职责。可按限界拆为：

- `ChatService`：run 编排、消息与标题；
- `CatalogService`：用户模型目录和缓存；
...
新增 `GET /api/tasks/:id/events` 前必须先校验 task 属主。鉴于当前 Bearer 鉴权，前端优先使用 fetch streaming SSE；若未来改为 HttpOnly Cookie，再考虑原生 EventSource。

事件流需要 initial snapshot、心跳、完成事件和断线后的 DB 回退。Redis pub/sub 只提供实时通知，不提供历史 replay。
...
不是 `migrate()` 本身有 1400+ 行，而是整个 `packages/server/src/db/database.ts` 超过 1400 行，其中 inline migration 约 386 行。建议引入 `schema_migrations` + 顺序 migration 文件，或使用成熟 runner；DDL 与 backfill 分开执行，并为大表迁移设计锁和回滚策略。
...
消息使用 conversation 内递增 `seq`，避免依赖时间戳排序。

### 11. 前端流状态 reducer 化

将 `useChat` 中 title、text delta、tool、artifact、generation 和 terminal 事件统一交给纯 reducer；以 `runId/eventId/toolCallId` 作为身份，减少闭包状态、重复事件和 React key 碰撞。

### 12. 密钥、会话与资源治理

- API key 和 platform access token 做静态加密；
- session token 只存摘要；
- 登录、上传、聊天和生成实施分级 rate limit；
...
1. **用量与限额面板**  
   已有 usage summary/logs/balance 查询能力，但 daily/monthly limit 的写入、权限和语义尚需定义。先补限额更新 API、reservation 展示和管理员/用户权限，再做按天、按模型图表。

2. **任务中心**  
   当前不是“cancel API 已齐全”。需要新增用户级任务列表、分页、属主校验 cancel API、`cancelled` 终态、跨进程取消传播，再提供进行中/历史任务 UI。

3. **素材库 / 生成历史**  
   assets 有基础表和索引，但缺少完整的用户级分页列表、归档/删除、缓存命中归属与引用模型。完成后再提供瀑布流、下载、复制 prompt、重生成和作为参考图。

4. **记忆管理 UI**  
   memory 的 list/search/set/delete API 基本可复用。UI 应同时说明来源、更新时间和删除影响，让用户能查看并纠正系统记忆。

5. **确定性的会话导出**  
   `generate_document` 是 Agent 工具，不等同于可靠的会话导出。建议新增服务端导出接口，按属主校验后序列化消息、工具调用、附件和生成资产，并直接创建 md/docx/pdf artifact，避免再次调用 LLM。

6. **消息编辑与分支重发**  
   当前 regenerate 是破坏性删除。可引入 branch/parent message 关系，编辑任意 user 消息后创建新分支，保留原历史并允许切换。

### 中期：完成已有抽象的生产闭环

7. **RAG 知识库**  
   Retriever、InMemoryVectorStore 和 attachment extraction 已有基础。还需 pgvector/外部向量库、文档切分与版本、异步入库、删除传播、引用证据、租户隔离和配额管理。

8. **多 Agent 编排执行器**  
   在 `agentAsTool` 与 Pipeline 原语上补执行状态、超时/取消、预算传播、工具白名单、人工 gate、结果 schema 和 trace，避免把子 Agent 变成不可观测的黑盒。

9. **断线续传**  
   除事件 `seq` 外，还需要 `runId`、事件缓存/持久化、保留期限、Last-Event-ID 解析、重复去除和终态 replay。Redis pub/sub 本身不能承担历史事件存储。

10. **语音输入**  
    接入真实 ASR 前需补音频格式、时长/大小限制、上传隐私、取消和成本计量。

11. **自定义 Agent**  
    将 `AgentDefinition` 入库只是第一步，还需工具白名单、模型可用性校验、prompt/version 管理、租户隔离、发布审核、输出 schema 校验和版本回滚。

### 远期：依赖外部平台和组织能力

12. **真实发布通道**：平台 OAuth、token 加密、刷新/撤销、幂等发布、失败重试与 review audit。
13. **内容审核升级**：文本、图片、视频多模态审核，保留策略版本和人工复核记录。
14. **团队空间**：组织 RBAC、会话/资产共享边界、key 池与成本中心、审计日志。
15. **管理端可观测性**：修复 trace 越权后建立 admin-only 浏览器，提供耗时瀑布、错误聚合、成本 Top-N、模型与工具维度筛选。
