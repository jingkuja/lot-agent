...
- 开发版：微信开发者工具可勾选「不校验合法域名」，服务器地址在登录页或「我的」里改。
...
| 账房（我的） | 同一账户额度、服务器地址、退出 |
| 修图 | 先选照片再改；也可把参考图丢回印台 |
| 登录 | 手机验证码（主）/ 账号密码（RSA-OAEP） |

微信静默登录（可选）：配置 `WECHAT_MP_APPID` + `WECHAT_MP_SECRET` 后，
`POST /api/auth/wechat-login` 用 `wx.login` code 换会话。未绑定过的 openid 会拿到一次性
`ticket`，用户用手机号或密码登录后再 `POST /api/auth/wechat-bind`。
...
3. 登录页把服务器改成 `http://127.0.0.1:3000`（或你的盒子地址），勾选不校验域名。
