...
镜像由两份 Dockerfile **手动构建**（compose 只引用镜像名 `:1.0`，不负责构建）：

```bash
# 1) 构建 Node 应用镜像（server 与 worker 共用，靠 ROLE 区分）
docker build -t lot-agent-app:1.0 -f Dockerfile .

# 2) 构建 Web（nginx）镜像
docker build -t lot-agent-web:1.0 -f Dockerfile.web .
```

> 镜像名/标签必须与 `docker-compose.yml` 中的 `image:` 一致（`lot-agent-app:1.0`、`lot-agent-web:1.0`）。后续升版改 tag（如 `:1.1`）时，两边同步修改。

构建完成后启动：

```bash
# 后台启动全部服务（不带 --build，直接用已构建的镜像）
docker compose up -d
...
docker compose up -d              # 应用编排变更（镜像需先手动 build，见 §5）
...
```bash
git pull
# 重新构建镜像（可沿用 :1.0，或升 tag 如 :1.1 并同步改 docker-compose.yml）
docker build -t lot-agent-app:1.0 -f Dockerfile .
docker build -t lot-agent-web:1.0 -f Dockerfile.web .
docker compose up -d              # 用新镜像滚动更新；server 启动自动跑增量迁移
```
