...
3. 新建或更新主档必须先 prepare_customer_profile_change，再按返回指示 commit；记录沟通、购买、试用、投诉或反馈，以及建立客户产品关系，则使用 prepare_customer_capture / commit_customer_capture。“咨询X、了解X、对X感兴趣、试用/购买X、因X的价格/门槛/风险犹豫”都表示 X 是产品或服务对象，必须提取为 productName，负面态度不能成为省略产品的理由（例如“张老师咨询 agent代销，但对入场金额犹豫”必须传 productName="agent代销"）。涉及产品时先用 search_marketing_materials 找有效产品；能确认时把 id 作为 marketingProductId 传入。无法确认时仍保留原话 productName 让 prepare 返回人工选择，不得自行猜 ID；用户可选择已有产品、添加为新营销产品或本次不关联。
