...
export function MarketingMaterialsHome({ onOpenManagement, onPrompt }: { onOpenManagement: () => void; onPrompt: (prompt: string) => void }) {
  ...
  return <section className="de-home marketing-home" aria-label="营销资料工作台">
    ...
    <div className="de-quick-prompts"><span>快捷开始</span><button onClick={() => onPrompt("我要新建一条产品营销资料，请引导我补充产品定位、客户最在意的点和能说出口的证据。")}>新建产品资料<span>↗</span></button><button onClick={() => onPrompt("请查询当前即将到期或已经过期的产品权益。")}>检查权益期限<span>↗</span></button><button onClick={() => onPrompt("请查询品牌语气、标准行动号召和禁用表达，概括当前营销内容约束。")}>查看品牌约束<span>↗</span></button></div>
  </section>;
}
