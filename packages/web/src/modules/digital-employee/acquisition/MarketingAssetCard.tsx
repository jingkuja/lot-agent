...
export function MarketingAssetCard({ item, onView, onManage, onReuse, onArchive }: {
  ...
  return <article className="de-marketing-asset-card">
    <div className={`de-asset-preview asset-${item.assetType} clickable`} onClick={onView} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onView(); } }} title="查看内容详情">
      {item.fileUrl && item.assetType === "video" ? <video src={item.fileUrl} controls preload="metadata" onClick={(event) => event.stopPropagation()} /> : item.fileUrl ? <img src={item.fileUrl} alt={item.title} /> : item.assetType === "text" ? <p>{item.content.slice(0, 180)}</p> : <div className={`de-generation-state ${item.generationStatus}`}><span>{item.generationStatus === "failed" ? "!" : "◌"}</span>{item.generationStatus === "failed" ? "生成失败" : "正在生成"}</div>}
    ...
    <div className="de-asset-card-body">
      ...
      <div className="de-asset-card-actions">
        <button className="de-secondary-button" onClick={onView}>查看内容</button>
        {item.fileUrl && <a className="de-secondary-button" href={item.fileUrl} download>下载</a>}
        <button className="de-secondary-button" onClick={onReuse}>复用</button>
        <button className="de-primary-button" disabled={item.generationStatus !== "ready"} onClick={onManage}>投放与反馈</button>
        <button className="de-icon-button" title="归档" onClick={onArchive}>⋯</button>
      </div>
    </div>
  </article>;
}
