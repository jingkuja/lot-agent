...
const navigation = [
  { id: "all", label: "全部资料", path: "M10 3a7 7 0 1 0 0 14 7 7 0 0 0 0-14 M15 15l6 6" },
...
export function KnowledgePanel({ onClose, onUse }: { onClose: () => void; onUse?: (file: File) => void }) {
  ...
  const [selected, setSelected] = useState("all"); const selection = useRef(selected); selection.current = selected;
  ...
  return <dialog ref={dialog} className="knowledge-dialog" onCancel={(event) => { event.preventDefault(); close(); }} aria-labelledby="knowledge-title">
    ...
    <div className="knowledge-layout"><aside className="knowledge-libraries"><nav aria-label="知识导航">
      ...
      {navigation.map(({ id, label, path }) => <button className="knowledge-nav-item" key={id} aria-current={selected === id || (id === "all" && selected === "inbox") ? "page" : undefined} disabled={busy} onClick={() => setSelected(id)}><KnowledgeIcon path={path} /><span>{label}</span></button>)}
    ...
    </nav><button className="knowledge-create-library" disabled={busy} onClick={() => { setSelected("all"); setNewLibrary(true); }}><span aria-hidden="true">＋ </span>新建知识库</button>
      ...
      {selected === "profile" ? <ProfilePanel collections={collections} /> : <>
        <div className="knowledge-head knowledge-section-heading"><div><h3>{collection?.name ?? (selected === "materials" ? "个人素材" : "全部资料")}</h3><p>{collection?.description || (selected === "materials" ? "集中管理图片、音频和视频，随时用于对话。" : "查找已保存的内容，检索相关片段与原文出处。")}</p></div>{collection && <button disabled={busy} onClick={() => setEditingCollection({ ...collection })}>编辑知识库</button>}</div>
        ...
        {(selected === "all" || selected === "inbox") && <div className="knowledge-scope-filter" role="group" aria-label="资料分类筛选">
          <button type="button" aria-pressed={selected === "all"} disabled={busy} onClick={() => setSelected("all")}>全部</button>
        ...
        {selected === "materials" ? <section><p>旧素材归档会复制到私有空间，不会撤销原有分享链接；分享仍由原功能管理。图片支持文字识别，无文字时自动补充图片说明用于检索；音视频可编辑说明用于检索。</p><label>来源<select value={materialSource} onChange={(e) => setMaterialSource(e.target.value)}><option value="">全部来源</option><option value="upload">用户上传</option><option value="generated">Agent 生成</option><option value="archived">已归档</option></select></label><label>加入知识库<select value={target} onChange={(e) => setTarget(e.target.value)}><option value="">选择知识库</option>{collections.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          ...
          </div><h4>私有个人资料</h4>{items.map((i) => <Fragment key={i.id}><article className="knowledge-item"><div><strong>{i.title}</strong><p>{types[i.sourceType]} · {i.materialSource === "generated" ? "Agent 生成" : "用户上传 / 私有保存"}{i.sourceAssetId ? " · 已归档" : ""} · {i.tags.join(" · ")}</p></div><div className="knowledge-actions">{editing?.id === i.id ? renderEditingActions(i.id) : <button onClick={() => void showSource(i)}>预览</button>}<button disabled={busy || !target} onClick={() => void act(() => knowledgeApi.memberships([i.id], [target], true))}>加入知识库</button>{i.sourceType !== "profile_fact" && editing?.id !== i.id && <button disabled={busy} aria-expanded={editing?.id === i.id} aria-controls={editing?.id === i.id ? `knowledge-editor-${i.id}` : undefined} onClick={() => setEditing({ ...i })}>编辑说明</button>}{onUse && <button disabled={busy} onClick={() => void useItem(i)}>用于当前对话</button>}</div></article>{renderEditor(i.id)}</Fragment>)}
