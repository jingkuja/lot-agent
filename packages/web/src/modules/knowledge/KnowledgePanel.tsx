...
export function KnowledgePanel({ onClose, onUse }: { onClose: () => void; onUse?: (file: File) => void }) {
  ...
  return <dialog ref={dialog} className="knowledge-dialog" onCancel={(event) => { event.preventDefault(); close(); }} aria-labelledby="knowledge-title">
    ...
    <div className="knowledge-layout"><aside className="knowledge-libraries"><nav aria-label="知识导航">
      ...
      {navigation.map(({ id, label, path }) => <button className="knowledge-nav-item" key={id} aria-current={selected === id || (id === "libraries" && (selected === "inbox" || !!library(selected))) ? "page" : undefined} disabled={busy} onClick={() => setSelected(id)}><KnowledgeIcon path={path} /><span>{label}</span></button>)}
    </nav>
      <small>搜索快捷键：⌘ / Ctrl + K</small>
    </aside><main className="knowledge-main">
      {selected === "profile" ? <ProfilePanel collections={collections} /> : <>
        <div className="knowledge-head knowledge-section-heading"><div><h3>{collection?.name ?? (selected === "materials" ? "个人素材" : selected === "inbox" ? "未分类资料" : "知识库")}</h3><p>{collection?.description || (selected === "materials" ? "集中管理图片、音频和视频，随时用于对话。" : selected === "inbox" ? "尚未加入任何知识库的资料，可批量加入知识库。" : selected === "libraries" ? `共 ${collections.length} 个知识库，点击图标进入；下方是全部资料的检索与列表。` : "查找已保存的内容，检索相关片段与原文出处。")}</p></div>{collection ? <div className="knowledge-heading-actions"><button disabled={busy} onClick={() => setSelected("libraries")}>返回知识库</button><button disabled={busy} aria-expanded={!!editingCollection} onClick={() => setEditingCollection(editingCollection ? null : { ...collection })}>{editingCollection ? "取消编辑" : "编辑知识库"}</button></div> : selected !== "materials" && <button disabled={busy} onClick={() => setCreatingLibrary(true)}>新建知识库</button>}</div>
        {selected === "libraries" && <>{creatingLibrary && <form className="knowledge-collect" aria-label="新建知识库" onSubmit={(e) => { e.preventDefault(); void act(async () => { const created = await knowledgeApi.createCollection(name.trim()); setName(""); setCreatingLibrary(false); setSelected(created.id); }); }}><h3>新建知识库</h3><label>知识库名称<input required autoFocus maxLength={100} value={name} onChange={(e) => setName(e.target.value)} /></label><div className="knowledge-actions"><button className="knowledge-primary" disabled={busy}>创建知识库</button><button type="button" disabled={busy} onClick={() => setCreatingLibrary(false)}>取消</button></div></form>}
        <section className="knowledge-library-grid" aria-label="我的知识库">
          {collections.map((c) => <button type="button" className="knowledge-library-card" key={c.id} title={c.description || c.name} disabled={busy} onClick={() => setSelected(c.id)}><span className="knowledge-library-card-icon" aria-hidden="true"><KnowledgeIcon path={libraryIcon(c.id)} /></span><strong>{c.name}</strong><small>{c.storedCount} 份保存 · {c.searchableCount} 份可检索</small></button>)}
          <button type="button" className="knowledge-library-card knowledge-library-card-create" disabled={busy} onClick={() => setCreatingLibrary(true)}><span className="knowledge-library-card-icon" aria-hidden="true">＋</span><strong>新建知识库</strong><small>新建一个知识库归类资料</small></button>
          {collectionCursor && <button type="button" className="knowledge-library-card knowledge-library-card-more" disabled={busy} onClick={() => void knowledgeApi.collections(collectionCursor).then((p) => { setCollections((old) => [...old, ...p.data.filter((c) => !old.some((i) => i.id === c.id))]); setCollectionCursor(p.nextCursor); }).catch(report)}><span className="knowledge-library-card-icon" aria-hidden="true">⋯</span><strong>更多知识库</strong><small>加载下一页</small></button>}
        </section></>}
        {selected !== "materials" && <section aria-label="资料检索"><form className="knowledge-search" onSubmit={(e) => { e.preventDefault(); const scope = selected; setBusy(true); setError(""); setResults(null); setPreview(null); void knowledgeApi.search(library(scope), query, mode, degraded, type, tag).then((value) => { if (selection.current === scope) setResults(value); }).catch(report).finally(() => setBusy(false)); }}>
          <label>{library(selected) ? "在当前知识库检索" : "搜索全部个人资料（含未分类资料与有效个人信息）"}<input ref={searchInput} value={query} onChange={(e) => setQuery(e.target.value)} maxLength={2000} placeholder="输入问题或关键词" required /></label><label>方式<select aria-label="方式" value={mode} onChange={(e) => setMode(e.target.value)}><option value="hybrid">混合</option><option value="keyword">关键词</option><option value="semantic">语义</option></select></label><button className="knowledge-primary" disabled={busy || !enabled || !query.trim()}>检索</button><label className="knowledge-checkbox"><input type="checkbox" checked={degraded} onChange={(e) => setDegraded(e.target.checked)} />语义服务不可用时允许关键词检索</label>
        </form>{results && <div className="knowledge-results" aria-live="polite">{results.degraded && <p>语义服务暂不可用，本次使用关键词检索。</p>}{!results.results.length && <p>当前范围内没有命中资料。</p>}{results.results.map((hit) => <article key={hit.chunkId}><h4>{hit.title}</h4><small>{types[hit.sourceType]} · {locator(hit)}{hit.origin === "manual_description" ? " · 来自手工说明" : hit.origin === "ocr" ? " · 来自文字识别" : hit.origin === "generated_description" ? " · 来自 AI 图片说明" : ""}</small><p>{hit.content}</p><div className="knowledge-actions"><button onClick={() => void knowledgeApi.item(hit.itemId).then((item) => showSource(item, hit)).catch(report)}>查看出处</button><button onClick={() => void (async () => { const current = await knowledgeApi.item(hit.itemId); if (current.activeRevisionId !== hit.revisionId || (library(selected) && !current.collectionIds.includes(selected))) { setResults(null); setPreview(null); throw new Error("引用已失效，请重新检索。"); } await knowledgeApi.text(hit.itemId, hit.revisionId); await navigator.clipboard.writeText(hit.content); setNotice("已复制摘录"); })().catch(report)}>复制摘录</button></div></article>)}</div>}</section>}
        {editingCollection && <form className="knowledge-collect" onSubmit={(e) => { e.preventDefault(); void act(async () => { await knowledgeApi.updateCollection(editingCollection, editingCollection); setEditingCollection(null); }); }}><label>库名称<input required maxLength={100} value={editingCollection.name} onChange={(e) => setEditingCollection({ ...editingCollection, name: e.target.value })} /></label><label>说明<textarea maxLength={2000} value={editingCollection.description} onChange={(e) => setEditingCollection({ ...editingCollection, description: e.target.value })} /></label><label>标签（逗号分隔）<input defaultValue={editingCollection.tags.join(", ")} onBlur={(e) => setEditingCollection({ ...editingCollection, tags: tags(e.target.value) })} /></label><div className="knowledge-actions"><button disabled={busy}>保存库信息</button><button type="button" disabled={busy} onClick={() => { if (window.confirm("删除此知识库？资料会保留在全部资料中。")) void act(async () => { await knowledgeApi.deleteCollection(editingCollection); setCollections((old) => old.filter((c) => c.id !== editingCollection.id)); setSelected("libraries"); }); }}>删除库，保留资料</button></div></form>}
        {collection && <section className="knowledge-collect" aria-label="导入资料" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (!busy) pick(Array.from(e.dataTransfer.files)); }}>
          <label className={`knowledge-dropzone${busy ? " disabled" : ""}`}>
            ...
            <span>导入到知识库「{collection.name}」：支持 PDF、Word、文本及图片、音视频，每批最多 20 个文件</span>
          ...
          <div className="knowledge-actions"><button className="knowledge-primary" disabled={busy || !uploads.some((u) => u.status === "waiting")} onClick={() => void uploadBatch(uploads.filter((u) => u.status === "waiting"))}>保存资料</button>{uploads.some((u) => u.status === "failed") && <button disabled={busy} onClick={() => void uploadBatch(uploads.filter((u) => u.status === "failed"))}>仅重试上传失败项</button>}</div>
          <details><summary>粘贴文字 / 添加书签</summary><form onSubmit={(e) => { e.preventDefault(); void act(async () => { await knowledgeApi.createItem({ sourceType: kind, title: title.trim(), ...(kind === "note" ? { content: body } : { sourceUrl: url, description: body }), collectionIds: [selected] }); setTitle(""); setBody(""); setUrl(""); }); }}><label>资料类型<select value={kind} onChange={(e) => setKind(e.target.value)}><option value="note">笔记</option><option value="bookmark">书签（仅保存链接和说明）</option></select></label><label>标题<input required maxLength={255} value={title} onChange={(e) => setTitle(e.target.value)} /></label>{kind === "bookmark" && <label>网址<input required type="url" value={url} onChange={(e) => setUrl(e.target.value)} /></label>}<label>{kind === "note" ? "正文" : "说明"}<textarea required={kind === "note"} maxLength={kind === "note" ? 200000 : 5000} rows={5} value={body} onChange={(e) => setBody(e.target.value)} /></label><button disabled={busy}>保存{kind === "note" ? "笔记" : "书签"}</button></form></details>
        </section>}
        {(selected === "libraries" || selected === "inbox") && <div className="knowledge-scope-filter" role="group" aria-label="资料分类筛选">
          <button type="button" aria-pressed={selected === "libraries"} disabled={busy} onClick={() => setSelected("libraries")}>全部</button>
          <button type="button" aria-pressed={selected === "inbox"} disabled={busy} onClick={() => setSelected("inbox")}>未分类</button>
          {selected === "inbox" && <span>尚未加入任何知识库的资料</span>}
        </div>}
        ...
        {selected === "materials" ? <section><p>旧素材归档会复制到私有空间，不会撤销原有分享链接；分享仍由原功能管理。图片支持文字识别，无文字时自动补充图片说明用于检索；音视频可编辑说明用于检索。</p><label>来源<select value={materialSource} onChange={(e) => setMaterialSource(e.target.value)}><option value="">全部来源</option><option value="upload">用户上传</option><option value="generated">Agent 生成</option><option value="archived">已归档</option></select></label><label>加入知识库<select value={target} onChange={(e) => setTarget(e.target.value)}><option value="">选择知识库</option>{collections.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          ...
          {!materials.length && <div className="knowledge-empty"><KnowledgeIcon path="M3 4h18v16H3z M3 16l5-5 4 4 3-3 6 6 M16 8h.01" /><strong>还没有素材</strong><p>对话中生成的图片、音频和视频会出现在这里。</p></div>}
        ...
        <section className="knowledge-documents" aria-label="资料列表"><div className="knowledge-head"><h3>资料 <span className="knowledge-count">{items.length}</span></h3><button disabled={busy} onClick={() => void refresh(true).catch(report)}>刷新</button></div><div className="knowledge-actions"><label className="knowledge-checkbox"><input type="checkbox" checked={items.length > 0 && checked.length === items.length} onChange={(e) => setChecked(e.target.checked ? items.map((i) => i.id) : [])} />选择本页已加载资料（{checked.length}）</label><select aria-label="批量目标知识库" value={target} onChange={(e) => setTarget(e.target.value)}><option value="">选择知识库</option>{collections.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select><button disabled={busy || !checked.length || !target} onClick={() => void act(() => knowledgeApi.memberships(checked, [target], true))}>加入库</button>{library(selected) && <button disabled={busy || !checked.length} onClick={() => void act(async () => { await knowledgeApi.memberships(checked, [selected], false); setChecked([]); })}>移出当前库</button>}<button disabled={busy || !enabled || !items.some((i) => i.indexStatus === "failed")} onClick={() => void act(async () => { const failed = await Promise.allSettled(items.filter((i) => i.indexStatus === "failed").map((i) => knowledgeApi.retry(i))); setNotice(`已提交 ${failed.filter((r) => r.status === "fulfilled").length} 项重试；${failed.filter((r) => r.status === "rejected").length} 项未成功，请刷新后检查。`); })}>仅重试处理失败项</button></div>
          {!items.length && <div className="knowledge-empty"><KnowledgeIcon path="M4 3h16v18H4z M8 8h8 M8 12h8 M8 16h4" /><strong>{selected === "inbox" ? "暂无未分类资料" : "还没有资料"}</strong><p>{selected === "inbox" ? "当前筛选下没有未分类资料，可切换到“全部”查看。" : collection ? "用上方导入添加文件，或在「个人素材」归档内容。" : "进入知识库可导入文件，或在「个人素材」归档内容。"}</p></div>}{items.map((item) => <Fragment key={item.id}><article className="knowledge-item"><label className="knowledge-checkbox"><input type="checkbox" checked={checked.includes(item.id)} onChange={(e) => setChecked((old) => e.target.checked ? [...old, item.id] : old.filter((id) => id !== item.id))} /><span><strong>{item.title}</strong><p>{types[item.sourceType]} · {enabled ? states[item.indexStatus] ?? item.indexStatus : "已保存"}{item.activeRevisionId && item.pendingRevisionId ? " · 旧版仍可检索" : ""}{item.indexStatus === "processing" ? ` · ${tasks[item.id]?.progress ?? 0}%` : ""}</p><small>{item.tags.join(" · ")}{["audio", "video"].includes(item.sourceType) ? " · 仅索引手工说明，未识别媒体内容" : ""}</small><Diagnostics warnings={item.diagnostics?.warnings} />{item.indexStatus === "failed" && <p>{ingestionErrors[item.errorCode ?? tasks[item.id]?.error ?? ""] ?? `错误代码：${item.errorCode ?? tasks[item.id]?.error ?? "请重试或检查文件格式"}`}</p>}</span></label><div className="knowledge-actions">{editing?.id === item.id ? renderEditingActions(item.id) : <><button onClick={() => void showSource(item)}>查看</button>{item.sourceType !== "profile_fact" && <button disabled={busy} aria-expanded={editing?.id === item.id} aria-controls={editing?.id === item.id ? `knowledge-editor-${item.id}` : undefined} onClick={() => setEditing({ ...item })}>编辑 / 替换</button>}</>}{onUse && <button disabled={busy} onClick={() => void useItem(item)}>用于当前对话</button>}{enabled && ["failed", "cancelled"].includes(item.indexStatus) && <button disabled={busy} onClick={() => void act(() => knowledgeApi.retry(item))}>重试</button>}{enabled && ["pending", "processing"].includes(item.indexStatus) && item.taskId && <button disabled={busy} onClick={() => void act(() => knowledgeApi.cancel(item.taskId!))}>取消</button>}{item.sourceType !== "profile_fact" && <button disabled={busy} onClick={() => { const names = item.collectionIds.map((id) => collections.find((c) => c.id === id)?.name ?? id).join("、"); if (window.confirm(`删除“${item.title}”？会从所有关联知识库中移除：${names || "无"}。`)) void act(() => knowledgeApi.remove(item)); }}>删除</button>}</div></article>{renderEditor(item.id)}</Fragment>)}
