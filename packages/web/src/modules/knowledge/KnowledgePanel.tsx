...
import { DuplicateFileError, knowledgeApi, type Collection, type Item, type Evidence, type Results, type Source, type Material } from "./api.js";
...
const states: Record<string, string> = { pending: "等待处理", processing: "正在处理", ready: "可检索", failed: "处理失败", cancelled: "已取消" };
...
const library = (scope: string) => !["inbox", "all", "profile", "materials", "integrations"].includes(scope) ? scope : "";
function locator(hit: Evidence) { const c = hit.citation; return c?.kind === "pdf" ? `第 ${c.page} 页` : c?.kind === "docx" ? `${c.heading ? `${c.heading} · ` : ""}第 ${c.paragraph} 段` : c?.kind === "text" ? `第 ${c.startLine}–${c.endLine} 行` : "资料说明"; }
...
interface Preview { item: Item; revision: string; source: Source; hit?: Evidence; url?: string }
export function KnowledgePanel({ onClose, onUse }: { onClose: () => void; onUse?: (file: File) => void }) {
  ...
  const refresh = useCallback(async (preserve = false) => {
    ...
    if (selected === "profile" || selected === "integrations") return;
    if (selected === "materials") {
      const [page, privatePage] = await Promise.all([knowledgeApi.materials(), knowledgeApi.items("all", undefined, type, tag, filter)]);
  ...
  useEffect(() => { setItems([]); setChecked([]); setCursor(null); setResults(null); setPreview(null); setEditing(null); setEditingCollection(null); setError(""); void refresh().catch(report); }, [refresh]);
  useEffect(() => {
  ...
  return <dialog ref={dialog} className="knowledge-dialog" onCancel={(event) => { event.preventDefault(); close(); }} aria-labelledby="knowledge-title">
    <header className="knowledge-head"><div><h2 id="knowledge-title">本地知识库</h2><p>保存资料，检索片段，查看出处 · 占用 {(bytes / 1024 / 1024).toFixed(1)} MB</p></div><button onClick={close} disabled={busy} aria-label="关闭知识库">关闭</button></header>
    ...
    <div className="knowledge-layout"><aside className="knowledge-libraries"><nav aria-label="知识导航">
      {[["inbox", "收件箱"], ["all", "全部资料 / 搜索"], ["profile", "个人信息"], ["materials", "个人素材"], ["integrations", "外部接入"]].map(([id, label]) => <button key={id} aria-current={selected === id ? "page" : undefined} disabled={busy} onClick={() => setSelected(id)}>{label}</button>)}
    ...
    </nav><button disabled={busy} onClick={() => { setSelected("inbox"); setNewLibrary(true); }}>新建知识库</button>
      ...
      {selected === "integrations" ? <IntegrationsPanel collections={collections} /> : selected === "profile" ? <ProfilePanel collections={collections} /> : <>
        <div className="knowledge-head"><h3>{collection?.name ?? (selected === "inbox" ? "收件箱" : selected === "materials" ? "个人素材" : "全部资料")}</h3>{collection && <button disabled={busy} onClick={() => setEditingCollection({ ...collection })}>编辑知识库</button>}</div>
        ...
        {selected !== "materials" && <section className="knowledge-collect" aria-label="收集资料" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (!busy) pick(Array.from(e.dataTransfer.files)); }}>
          <label>选择文件，或拖放到这里<input type="file" accept={accept} multiple disabled={busy} onChange={(e) => { pick(Array.from(e.target.files ?? [])); e.target.value = ""; }} /></label>
        ...
        <div className="knowledge-filters"><label>类型<select value={type} onChange={(e) => setType(e.target.value)}><option value="">全部类型</option>{Object.entries(types).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>标签<input value={tag} onChange={(e) => setTag(e.target.value)} placeholder="精确标签" /></label><label>标题筛选<input value={filter} onChange={(e) => setFilter(e.target.value)} /></label></div>
        {selected === "materials" ? <section><p>旧素材归档会复制到私有空间，不会撤销原有分享链接；分享仍由原功能管理。媒体内容识别尚未启用，可编辑说明用于检索。</p><label>来源<select value={materialSource} onChange={(e) => setMaterialSource(e.target.value)}><option value="">全部来源</option><option value="upload">用户上传</option><option value="generated">Agent 生成</option><option value="archived">已归档</option></select></label><label>加入知识库<select value={target} onChange={(e) => setTarget(e.target.value)}><option value="">选择知识库</option>{collections.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          {materials.filter((m) => !tag && (!type || m.mime.startsWith(`${type}/`) || type === "document" && !/^(image|audio|video)\//.test(m.mime)) && (!filter || (m.original_name ?? m.id).includes(filter)) && (!materialSource || (materialSource === "archived" ? !!m.archived_item_id : materialSource === "upload" ? m.type === "upload" : m.type !== "upload"))).map((m) => <article className="knowledge-item" key={m.id}><div><strong>{m.original_name ?? `素材-${m.id}`}</strong><p>{m.mime} · {m.type === "upload" ? "上传" : "Agent 生成"} · {m.archived_item_id ? "已归档" : "未归档"}</p></div><div className="knowledge-actions"><button disabled={busy} onClick={() => void act(async () => { await knowledgeApi.archive(m, []); setNotice("已保存到私有个人素材。"); })}>保存个人素材</button><button disabled={busy || !target} onClick={() => void act(() => knowledgeApi.archive(m, [target]))}>加入知识库</button>{m.archived_item_id && <button disabled={busy} onClick={() => void knowledgeApi.item(m.archived_item_id!).then((i) => showSource(i)).catch(report)}>预览私有副本</button>}{onUse && <button disabled={busy} onClick={() => void act(async () => { onUse(await knowledgeApi.materialFile(m)); })}>用于当前对话</button>}</div></article>)}
          <h4>私有个人资料</h4>{items.filter((i) => !materialSource || materialSource === "archived" ? !materialSource || !!i.sourceAssetId : materialSource === "upload" ? !i.sourceAssetId : false).map((i) => <article className="knowledge-item" key={i.id}><div><strong>{i.title}</strong><p>{types[i.sourceType]} · {i.sourceAssetId ? "旧素材归档" : "私有保存"} · {i.tags.join(" · ")}</p></div><div className="knowledge-actions"><button onClick={() => void showSource(i)}>预览</button><button disabled={busy || !target} onClick={() => void act(() => knowledgeApi.memberships([i.id], [target], true))}>加入知识库</button>{i.sourceType !== "profile_fact" && <button onClick={() => setEditing({ ...i })}>编辑说明</button>}{onUse && <button disabled={busy} onClick={() => void useItem(i)}>用于当前对话</button>}</div></article>)}
          {cursor && <button disabled={busy} onClick={() => void knowledgeApi.items("all", cursor, type, tag, filter).then((p) => { setItems((old) => [...old, ...p.data]); setCursor(p.nextCursor); }).catch(report)}>更多私有资料</button>}
          {materialMore && <button disabled={busy} onClick={() => void knowledgeApi.materials(materials.at(-1)?.cursor_time, materials.at(-1)?.id).then((p) => { setMaterials((old) => [...old, ...p.data]); setMaterialMore(p.data.length === 30); }).catch(report)}>更多素材</button>}
      ...
      {preview && <SourcePreview value={preview} onClose={() => setPreview(null)} />}
    </main></div>
  </dialog>;
}
function SourcePreview({ value, onClose }: { value: Preview; onClose: () => void }) {
  const sourceRef = useRef<HTMLElement>(null);
  useEffect(() => { sourceRef.current?.scrollIntoView({ block: "start" }); }, [value]);
  ...
  return <section ref={sourceRef} className="knowledge-preview" aria-label="引用出处"><div className="knowledge-head"><h3>{item.title}{hit ? ` · ${locator(hit)}` : ""}</h3><button onClick={onClose}>收起</button></div>
    <div className="knowledge-source">{parts.map((block, index) => { const b = block.citation; const matches = !!hit && (b?.kind === c?.kind && (c?.kind === "pdf" ? b?.page === c.page : c?.kind === "docx" ? b?.paragraph === c.paragraph : c?.kind === "text" ? (b?.startLine ?? 0) <= (c.endLine ?? 0) && (b?.endLine ?? 0) >= (c.startLine ?? 0) : block.text.includes(hit.content))); return <pre key={index}>{matches ? <mark>{block.text}</mark> : block.text}</pre>; })}</div>
    {url && <>{item.mime?.startsWith("image/") && <img src={url} alt={item.title} />}{item.mime?.startsWith("audio/") && <audio controls src={url} onError={(e) => e.currentTarget.setAttribute("aria-label", "此编码无法播放，请下载原件")} />}{item.mime?.startsWith("video/") && <video controls src={url} />}{item.mime?.startsWith("audio/") || item.mime?.startsWith("video/") ? <p>若浏览器不支持此编码，请下载原件。预览票据过期后可重新打开。</p> : null}<a href={`${url}${c?.kind === "pdf" ? `#page=${c.page}` : ""}`} target="_blank" rel="noreferrer">打开 / 下载原件{c?.kind === "pdf" ? `（第 ${c.page} 页）` : ""}</a></>}
  </section>;
}
