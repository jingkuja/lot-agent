...
export function buildContent(slide: PptxSlide, s: PptSlide, ctx: BuildCtx): void {
  ...
  slide.addText(s.title, { x: W * 0.06, y: H * 0.07, w: W * 0.88, h: H * 0.13, fontFace: f.major, fontSize: 28, bold: true, color: ink.title });
  ...
  if (s.bullets?.length) {
    slide.addText(
      ...
      { x: W * 0.07, y: H * 0.27, w: W * 0.86, h: H * 0.6, fontFace: f.minor, fontSize: 16, color: ink.body, valign: "top", lineSpacingMultiple: 1.35 }
    );
  }
